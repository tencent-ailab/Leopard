...
from transformers import AutoTokenizer, GPT2Config, LlamaConfig, CLIPVisionConfig, LlavaConfig, AutoConfig, \
    SiglipVisionConfig
...
def convert_checkpoint_from_transformers_to_megatron(args):
    ...
    mm_projector_state_dict = torch.load(
        '/path_to/clip-vit-large-patch14-336/mm_projector_to_8B_siglip.pth',
        map_location="cpu")

    path_to_siglip = 'HuggingFaceM4/siglip-so400m-14-364-flash-attn2-navit'
    siglip = SiglipModel.from_pretrained(path_to_siglip)
    siglip_config = AutoConfig.from_pretrained(path_to_siglip)
    vision_config = siglip_config.vision_config
    vision_state_dict = siglip.state_dict()
    print(vision_state_dict['vision_model.embeddings.position_embedding.weight'].shape)
    ...
    for layer_id in range(config.num_hidden_layers):
        ...
        if layer_id < vision_config.num_hidden_layers:
            q_vision_weight = vision_state_dict[
                'vision_model.encoder.layers.' + str(layer_id) + '.self_attn.q_proj.weight']
            k_vision_weight = vision_state_dict[
                'vision_model.encoder.layers.' + str(layer_id) + '.self_attn.k_proj.weight']
            v_vision_weight = vision_state_dict[
                'vision_model.encoder.layers.' + str(layer_id) + '.self_attn.v_proj.weight']
            internal_state_dict[
                'vision_transformer.layers.' + str(layer_id) + '.self_attn.query_key_value.weight'] = torch.cat(
                (q_vision_weight, k_vision_weight, v_vision_weight))
            q_vision_bias = vision_state_dict['vision_model.encoder.layers.' + str(layer_id) + '.self_attn.q_proj.bias']
            k_vision_bias = vision_state_dict['vision_model.encoder.layers.' + str(layer_id) + '.self_attn.k_proj.bias']
            v_vision_bias = vision_state_dict['vision_model.encoder.layers.' + str(layer_id) + '.self_attn.v_proj.bias']
            internal_state_dict[
                'vision_transformer.layers.' + str(layer_id) + '.self_attn.query_key_value.bias'] = torch.cat(
                (q_vision_bias, k_vision_bias, v_vision_bias))
            internal_state_dict['vision_transformer.layers.' + str(layer_id) + '.self_attn.dense.weight'] = \
            vision_state_dict['vision_model.encoder.layers.' + str(layer_id) + '.self_attn.out_proj.weight']
            internal_state_dict['vision_transformer.layers.' + str(layer_id) + '.self_attn.dense.bias'] = \
            vision_state_dict['vision_model.encoder.layers.' + str(layer_id) + '.self_attn.out_proj.bias']
            internal_state_dict['vision_transformer.layers.' + str(layer_id) + '.self_attn.layer_norm.weight'] = \
            vision_state_dict['vision_model.encoder.layers.' + str(layer_id) + '.layer_norm1.weight']
            internal_state_dict['vision_transformer.layers.' + str(layer_id) + '.self_attn.layer_norm.bias'] = \
            vision_state_dict['vision_model.encoder.layers.' + str(layer_id) + '.layer_norm1.bias']
            internal_state_dict['vision_transformer.layers.' + str(layer_id) + '.mlp.dense_h_to_4h.weight'] = \
            vision_state_dict['vision_model.encoder.layers.' + str(layer_id) + '.mlp.fc1.weight']
            internal_state_dict['vision_transformer.layers.' + str(layer_id) + '.mlp.dense_h_to_4h.bias'] = \
            vision_state_dict['vision_model.encoder.layers.' + str(layer_id) + '.mlp.fc1.bias']
            internal_state_dict['vision_transformer.layers.' + str(layer_id) + '.mlp.dense_4h_to_h.weight'] = \
            vision_state_dict['vision_model.encoder.layers.' + str(layer_id) + '.mlp.fc2.weight']
            internal_state_dict['vision_transformer.layers.' + str(layer_id) + '.mlp.dense_4h_to_h.bias'] = \
            vision_state_dict['vision_model.encoder.layers.' + str(layer_id) + '.mlp.fc2.bias']
            internal_state_dict['vision_transformer.layers.' + str(layer_id) + '.mlp.layer_norm.weight'] = \
            vision_state_dict['vision_model.encoder.layers.' + str(layer_id) + '.layer_norm2.weight']
            internal_state_dict['vision_transformer.layers.' + str(layer_id) + '.mlp.layer_norm.bias'] = \
            vision_state_dict['vision_model.encoder.layers.' + str(layer_id) + '.layer_norm2.bias']
    ...
    internal_state_dict['vision_transformer.patch_embedding.weight'] = vision_state_dict[
        'vision_model.embeddings.patch_embedding.weight']
    internal_state_dict['vision_transformer.position_embedding.weight'] = vision_state_dict[
        'vision_model.embeddings.position_embedding.weight']
    if args.vision == 'clip':
        internal_state_dict['vision_transformer.class_embedding.weight'] = vision_state_dict[
            'vision_model.embeddings.class_embedding']
        internal_state_dict['vision_transformer.pre_layernorm.weight'] = vision_state_dict[
            'vision_model.pre_layrnorm.weight']
        internal_state_dict['vision_transformer.pre_layernorm.bias'] = vision_state_dict[
            'vision_model.pre_layrnorm.bias']
    elif args.vision == 'siglip':
        internal_state_dict['vision_transformer.patch_embedding.bias'] = vision_state_dict[
            'vision_model.embeddings.patch_embedding.bias']
        internal_state_dict['vision_transformer.post_layernorm.weight'] = vision_state_dict[
            'vision_model.post_layernorm.weight']
        internal_state_dict['vision_transformer.post_layernorm.bias'] = vision_state_dict[
    ...
    for k, v in megatron_args.items():
        setattr(margs, k, v)
    print('target params dtype', args.target_params_dtype)
    # params dtype
    ...
    for i in range(args.target_tensor_model_parallel_size):
        ...
        vision_enc_dict["vision_tower.position_embeddings.weight"] = state_dict[
            'vision_transformer.position_embedding.weight']
        if args.vision == 'clip':
            vision_enc_dict["vision_tower.class_token"] = state_dict[
                'vision_transformer.class_embedding.weight'].unsqueeze(0).unsqueeze(0)
            ...
            vision_enc_dict["vision_tower.conv1.bias"] = state_dict['vision_transformer.patch_embedding.bias']
            vision_enc_dict["vision_tower.ln_post.weight"] = state_dict['vision_transformer.post_layernorm.weight']
            vision_enc_dict["vision_tower.ln_post.bias"] = state_dict['vision_transformer.post_layernorm.bias']
    ...
    print('num layers', num_layers, 'vision layers', vision_config.num_hidden_layers)
    ...
    for pp_rank in range(args.target_pipeline_model_parallel_size):
        ...
        if pp_rank == 0:
            for layer in range(vision_config.num_hidden_layers):
                vision_layers_to_copy = [
                    layer_name
                    for layer_name in state_dict.keys()
                    if layer_name.startswith(f"vision_transformer.layers.{layer}.")
                ]
                for layer_name in vision_layers_to_copy:
                    ...
                    for i in range(args.target_tensor_model_parallel_size):
                        params_dict = get_element_from_dict_by_path(output_state_dict[i],
                                                                    "model.language_model.vision_tower")
            ...
            for layer_name in layers_to_copy:
                ...
                if op_name.startswith("input_layernorm") or op_name.startswith("post_attention_layernorm"):
                    ...
                    layer_name = f"layers.{layer}.self_attention.rotary_emb.inv_freq"
                        ...
                        "self_attn.query_key_value") and weight == "weight" and args.model_name != "llama2-70b":
                    # transformers stores D X (3*D) but Megatron-LM expects (3*D) X D.
                    params = transformers_to_megatron_fix_query_key_value_ordering(
                        params,
                        3.0,
                ...
                # handle attention K, V, Q weights
                    ...
                    layer_name = f"layers.{layer}.self_attention.query.{weight}"
                        ...
                        "self_attn.key_value") and weight == "weight" and args.model_name == "llama2-70b":
                    # transformers stores D X (3*D) but Megatron-LM expects (3*D) X D.
                    params = transformers_to_megatron_fix_query_key_value_ordering(
                        params,
                        3.0,
                ...
                if args.model_name != "llama2-70b":
                    ...
                    for i in range(args.target_tensor_model_parallel_size):
                        ...
                        params_dict[layer_name] = (
                            params[i].clone() if (
                                        op_name + "." + weight in tensor_parallel_params_70b) else params.clone()
                        )
            ...
            for i in range(args.target_tensor_model_parallel_size):
                ...
                if args.model_name == "llama2-70b":
                    ...
                    group_query_weight = query_weight.view(num_groups // args.target_tensor_model_parallel_size,
                                                           num_heads // num_groups * head_dim, hidden_size)
                    group_kv_weight = kv_weight.view(num_groups // args.target_tensor_model_parallel_size, 2 * head_dim,
                                                     hidden_size)
...
def convert_checkpoint_from_megatron_to_transformers(args):
    ...
    hf_config = LlamaConfig.from_pretrained('meta-llama/Llama-3.1-8B-Instruct')
    config = LlamaConfig(
    ...
    vision_config = SiglipVisionConfig.from_pretrained('HuggingFaceM4/siglip-so400m-14-364-flash-attn2-navit')
    llava_config = LlavaConfig(text_config=config, vision_config=vision_config, image_token_index=128200,
    ...
    for pp_rank in range(pp_size):
        ...
        for key, val in get_element_from_dict_by_path(tp_state_dicts[0], path).items():
            ...
            # Transpose the Q matrix for query for Llama70b.
                ...
                kv_dim = 2 * head_dim
                tp_states = [
                    i.view(num_groups // args.target_tensor_model_parallel_size, query_dim + kv_dim, hidden_size) for i
                    in tp_states]
        ...
        if pp_rank == 0:
            ...
            for key, val in get_element_from_dict_by_path(tp_state_dicts[0],
                                                          'model.language_model.vision_tower').items():
                if key.endswith('_extra_state'):
                    continue
                if args.vision == 'siglip':
                    key = 'vision_tower.' + key
                if key == "vision_tower.conv1.weight":
                    output_state_dict["vision_tower.vision_model.embeddings.patch_embedding.weight"] = val.to(
                        dtype).clone()
                elif key == 'vision_tower.conv1.bias':
                    output_state_dict["vision_tower.vision_model.embeddings.patch_embedding.bias"] = val.to(
                        dtype).clone()
                elif key == "vision_tower.position_embeddings.weight":
                    output_state_dict["vision_tower.vision_model.embeddings.position_embedding.weight"] = val.to(
                        dtype).clone()
                elif key == "vision_tower.class_token":
                    output_state_dict["vision_tower.vision_model.embeddings.class_embedding"] = val.to(
                        dtype).clone().squeeze()
                ...
                elif key == "vision_tower.ln_post.weight":
                    output_state_dict["vision_tower.vision_model.post_layernorm.weight"] = val.to(dtype).clone()
                elif key == "vision_tower.ln_post.bias":
                    output_state_dict["vision_tower.vision_model.post_layernorm.bias"] = val.to(dtype).clone()
                        ...
                        params = torch.cat(
                            ...
                            + [
                                get_element_from_dict_by_path(tp_state_dicts[tp_rank],
                                                              'model.language_model.vision_tower')[
                                    key.replace('vision_tower.', '') if args.vision == 'siglip' else key]
                                for tp_rank in range(1, tp_size)
                            ],
                            dim=dim,
                        ).to(dtype)
                    ...
                    if 'self_attention.linear_qkv' in key and 'layer_norm' not in key:
                        ...
                        if weight_or_bias == 'bias':
                            for index, matrix in enumerate(torch.split(out_val, out_val.shape[0] // 3, 0)):
                                output_state_dict[
                                    'vision_tower.vision_model.encoder.layers.' + layer_name + f".self_attn.{QKV[index]}.{weight_or_bias}"] = matrix.clone()
                        else:
                            for index, matrix in enumerate(torch.split(out_val, out_val.shape[1], 0)):
                                output_state_dict[
                                    'vision_tower.vision_model.encoder.layers.' + layer_name + f".self_attn.{QKV[index]}.{weight_or_bias}"] = matrix.clone()
                        ...
                        if key.endswith('weight'):
                            output_state_dict[
                                'vision_tower.vision_model.encoder.layers.' + layer_name + ".layer_norm1.weight"] = params.clone()
                        else:
                            output_state_dict[
                                'vision_tower.vision_model.encoder.layers.' + layer_name + ".layer_norm1.bias"] = params.clone()
                        ...
                        if key.endswith('weight'):
                            output_state_dict[
                                'vision_tower.vision_model.encoder.layers.' + layer_name + ".self_attn.out_proj.weight"] = params.clone()
                        else:
                            output_state_dict[
                                'vision_tower.vision_model.encoder.layers.' + layer_name + ".self_attn.out_proj.bias"] = params.clone()
                    elif 'mlp.linear_fc1' in key and 'layer_norm' not in key:
                        layer_name = key.split('mlp.linear_fc1')[0].split('.')[-2]
                        if key.endswith('weight'):
                            output_state_dict[
                                'vision_tower.vision_model.encoder.layers.' + layer_name + ".mlp.fc1.weight"] = params.clone()
                        else:
                            output_state_dict[
                                'vision_tower.vision_model.encoder.layers.' + layer_name + ".mlp.fc1.bias"] = params.clone()
                    elif 'mlp.linear_fc2' in key:
                        layer_name = key.split('mlp.linear_fc2')[0].split('.')[-2]
                        if key.endswith('weight'):
                            output_state_dict[
                                'vision_tower.vision_model.encoder.layers.' + layer_name + ".mlp.fc2.weight"] = params.clone()
                        else:
                            output_state_dict[
                                'vision_tower.vision_model.encoder.layers.' + layer_name + ".mlp.fc2.bias"] = params.clone()
                    elif 'mlp.linear_fc1' in key:
                        layer_name = key.split('mlp.linear_fc1')[0].split('.')[-2]
                        if key.endswith('weight'):
                            output_state_dict[
                                'vision_tower.vision_model.encoder.layers.' + layer_name + ".layer_norm2.weight"] = params.clone()
                        else:
                            output_state_dict[
                                'vision_tower.vision_model.encoder.layers.' + layer_name + ".layer_norm2.bias"] = params.clone()
                else:
    ...
    if args.vision == "clip":
        print('Adding final layers for CLIP')
        additional = {}
...
def convert_pp1_to_pp8():
    ...
    mlp_weight_0 = torch.cat(mlp_weight_0, dim=0)
    mlp_weight_0 = mlp_weight_0.to(dtype)
    output_state_dict["0.weight"] = mlp_weight_0.clone()
    mlp_bias_0 = torch.cat(mlp_bias_0, dim=0)
    mlp_bias_0 = mlp_bias_0.to(dtype)
    output_state_dict["0.bias"] = mlp_bias_0.clone()
    mlp_weight_1 = torch.cat(mlp_weight_1, dim=1)
    mlp_weight_1 = mlp_weight_1.to(dtype)
    output_state_dict["2.weight"] = mlp_weight_1.clone()
    output_state_dict["2.bias"] = mlp_bias_1.clone()
    for k, v in output_state_dict.items():
        print(k, v.shape)
    # Store the state_dict to file.
    torch.save(output_state_dict, os.path.join(args.load_path, 'mm_projector.pt'))
