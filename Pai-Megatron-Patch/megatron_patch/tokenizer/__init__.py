...
def get_special_token_id(token):
    return _GLOBAL_TOKENIZER.convert_tokens_to_ids(token)

def get_image_token_rank_and_id(image_token, gpu_num=8, vocab_size=0):
    assert vocab_size % gpu_num == 0, "vocab size is supposed to be divisible by num_gpu"

    per_gpu_vocab_size = vocab_size // gpu_num

    global_img_token_id = _GLOBAL_TOKENIZER.convert_tokens_to_ids(image_token)

    # img id == the rank - 1
    gpu_id = global_img_token_id // per_gpu_vocab_size
    offset = global_img_token_id % per_gpu_vocab_size
...
def build_tokenizer(args):
    ...
    if args.patch_tokenizer_type == 'JiebaBPETokenizer':
        ...
        try:
            args.padded_vocab_size = len(tokenizer.vocab) + args.extra_vocab_size
        except:
            args.padded_vocab_size = tokenizer.vocab_size + args.extra_vocab_size
        ...
        ### FIXME:
        # special tokens should be added during checkpoint conversion.
        # consider removing it in the next version
        
        if args.extra_vocab_size > 0:
        ...
        print ('eod token', tokenizer.eod, 'padded vocab size', tokenizer.vocab_size + args.extra_vocab_size, 'vocab size', tokenizer.vocab_size)
        # args.padded_vocab_size = tokenizer.vocab_size + args.extra_vocab_size

        # if 'llama-3' in args.load.lower() or 'llama3' in args.load.lower(): # TODO: This is a hack to fix the vocab size for llama-3, need to find a better way to do this
        #     # This is because the added tokens are not added to the vocab_size
        #     # https://stackoverflow.com/questions/67412925/what-is-the-difference-between-lentokenizer-and-tokenizer-vocab-size
        #     args.padded_vocab_size += 256 # llama-3 256 reservsed tokens
