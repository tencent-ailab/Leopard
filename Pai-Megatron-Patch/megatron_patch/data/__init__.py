...
from .llama import LLamaRawDataset, CKDataset
# from .bloom import BloomRawDataset
from .llava.mm_pretrain_dataset import LazySupervisedDataset as LLavaSupervisedDataset
from .idefics2.mm_pretrain_dataset import LazySupervisedDataset as Idefics2SupervisedDataset
from .idefics2.mm_pretrain_dataset import LazyVQADataset as Idefics2VQADataset
from .llava.mm_pretrain_dataset import get_sft_dataset
...
def build_finetune_dataset(dataset):
    ...
    build_tokenizer(args)
    if dataset == 'CK':
        train_dataset = CKDataset(args.train_data_path, args.max_padding_length)
        valid_dataset = CKDataset(args.valid_data_path, args.max_padding_length)
...
def build_pretrain_dataset_from_original(dataset):
    ...
    if dataset == 'CK':
        ...
        return train_dataset, train_dataset, train_dataset
    elif dataset == 'Idefics2-Pretrain-Raw':
        train_dataset = Idefics2SupervisedDataset(args.train_data_path)
        #valid_dataset = LLavaSupervisedDataset(args.valid_data_path)
        #test_dataset = LLavaSupervisedDataset(args.test_data_path)

        return train_dataset, train_dataset, train_dataset
    elif dataset == 'Idefics2-DocVQA-test':
        train_dataset = Idefics2VQADataset(args.train_data_path, "train")
        valid_dataset = Idefics2VQADataset(args.train_data_path, "test")
        #test_dataset = LLavaSupervisedDataset(args.test_data_path)

        return train_dataset, train_dataset, train_dataset
    
    elif dataset == 'LLava-SFT-Raw':
        from .llava.mm_pretrain_dataset import get_sft_dataset
        train_dataset = get_sft_dataset(args.train_data_path[0])
        return train_dataset, train_dataset, train_dataset
    
    elif dataset == 'Idefics2-SFT-Raw':
        from .idefics2.mm_pretrain_dataset import get_sft_dataset
        train_dataset = get_sft_dataset(args.train_data_path[0])
        return train_dataset, train_dataset, train_dataset
