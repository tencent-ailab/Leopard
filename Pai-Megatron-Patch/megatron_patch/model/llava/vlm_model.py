...
from megatron.legacy.model.utils import get_linear_layer, init_method_normal, scaled_init_method_normal
...
from .clip_encoder import CLIPVisionTowerMegatron, get_siglip_vision_tower
from .mm_projector_builder import build_vision_projector_megatron
...
class TransformerLanguageModel(MegatronModule):
    ...
    def __init__(self,
        ...
        if self.pre_process:
            ...
            if self.args.vision_tower.endswith('clip-vit-large-patch14-336'):
                self.vision_tower = CLIPVisionTowerMegatron(self.args.vision_tower, config)
                self.args.mm_hidden_size = self.vision_tower.vision_config.hidden_size
                self.mm_projector = build_vision_projector_megatron(config, self.vision_tower.vision_config, scale_factor=1)
                print ('BUILT CLIP VISION TOWER and projector')
            else:
                print (self.args.vision_tower)
                self.vision_tower = get_siglip_vision_tower(self.args.vision_tower, config)
                self.args.mm_hidden_size = self.vision_tower.config.hidden_size
                scale_factor = 4
                # if '364' in self.args.vision_tower:
                #     scale_factor = 4
                self.mm_projector = build_vision_projector_megatron(config, self.vision_tower.config, scale_factor=scale_factor)
        ...
        if self.args.use_rotary_position_embeddings:
            ...
            self.rotary_pos_emb = RotaryEmbedding(
                ...
                rotary_base=self.args.rotary_base, rotary_percent=self.args.rotary_percent,
                rope_type='llama3.1' if 'llama-3.1' in self.args.load else 'default'
            )
    ...
    def encode_images(self, images):
        image_features = self.vision_tower(images).contiguous()
        # if mpu.get_tensor_model_parallel_rank() == 0:
        #     print ('before MM projector', image_features.shape, image_features[0, 0:5, 0:2])
        # if self.args.vision_tower.endswith('siglip-so400m-14-364-flash-attn2-navit'):
    ...
    def forward(self, enc_input_ids, enc_position_ids, enc_attn_mask,
        ...
        if self.pre_process:
            ...
            if images.shape[-1] > 100:
                ...
                # if mpu.get_tensor_model_parallel_rank() == 0:
                #     print ('input_ids', enc_input_ids[0, 725:740])
                #     print ('images', images.shape, images[0, 0, 0:5, 0:2])
                #     print ('image_features', image_features.shape, image_features[0, 0:5, 0:2])
                #print (f"input_embeds {input_embeds.shape}", f"image_features {image_features.shape}")
        ...
        #         new_input_embeds.append(cur_input_embeds.unsqueeze(1))
        #         continue
        ...
        #         #print ('curr start', cur_start, 'image token start', image_token_start)
        ...
        #         else:
        #             cur_new_input_embeds.append(cur_input_embeds[cur_start:cur_start+image_token_start].unsqueeze(1))
        #             cur_new_input_embeds.append(cur_image_features)
        #             cur_start += image_token_start + 1
        #         padding_squeeze += cur_image_features.shape[0] - 1
        ...
        #     cur_new_input_embeds = cur_new_input_embeds[:cur_new_input_embeds.shape[0] - padding_squeeze]
        #     new_input_embeds.append(cur_new_input_embeds)
        #     enc_attn_mask[batch_idx] = torch.cat([torch.ones(padding_squeeze, device=cur_new_input_embeds.device), enc_attn_mask[batch_idx][:-padding_squeeze]], dim=0)
        # encoder_input = torch.cat(new_input_embeds, dim=1)

        #if enc_attn_mask is not None:
        #print (f'enc_attn_mask {enc_attn_mask.shape}', f'encoder_input {encoder_input.shape}')
        ...
        if self.use_rotary_position_embeddings:
            ...
            # print ('USE rotary_position_emb', rotary_pos_emb.shape, encoder_input.shape)
            # print (rotary_pos_emb[:10, :10])
        ...
        if self.pre_process:
            state_dict_[self._embedding_key] \
                = self.embedding.state_dict_for_save_checkpoint(prefix=prefix,
                                                                keep_vars=keep_vars)
            state_dict_['vision_tower'] = self.vision_tower.state_dict_for_save_checkpoint(prefix=prefix, keep_vars=keep_vars)
    ...
    def load_state_dict(self, state_dict, strict=True):
        ...
        if self.pre_process:
            ...
            state_dict_vision = state_dict['vision_tower']
            for k, v in self.vision_tower.state_dict().items():
                if k.endswith('_extra_state'):
                    state_dict_vision[k] = v
            if self.args.vision_tower.endswith('clip-vit-large-patch14-336'):
                state_dict_vision = {k:v for k, v in state_dict_vision.items() if not k.startswith('vision_tower.transformer.layers.23')}
            else:
                state_dict_vision = {k.replace('vision_tower.', ''):v for k, v in state_dict_vision.items()}
            # print([k for k,v in  self.vision_tower.state_dict().items()])
            # exit()
            self.vision_tower.load_state_dict(state_dict_vision, strict=strict)
